...
_PROVIDERS = {
    ...
    "7": ("apple_fm", "Apple Foundation Models (on-device)", None),
    "8": ("custom", "Custom / Self-hosted OpenAI-compatible API", None),
}
...
def _step2_provider() -> tuple[str, str, str, str]:
    ...
    if choice not in _PROVIDERS:
        _print(
            f"  Warning: '{choice}' is not a valid provider choice "
            f"(valid: {', '.join(_PROVIDERS.keys())}). Defaulting to Ollama.",
            style="yellow",
        )
        choice = "1"
    ...
    elif provider == "apple_fm":
        base_url = ""
        api_key = ""
        model = _ask("Model name", default=_DEFAULT_MODELS["apple_fm"])
    ...
    try:
        ...
        _print(
            f"  Warning: connectivity check failed ({type(exc).__name__}): {exc}",
            style="yellow",
        )
...
def _step7_scaffold(
    ...
    # Build config as a dict, then serialize with PyYAML if available.
    config: dict[str, Any] = {
        "model": model,
        "provider": provider,
    }
    ...
    config["sandbox_level"] = sandbox
    config["system_prompt_extra"] = (
        f"You are {personalize.get('agent_name', 'OllamaCode')}, a helpful coding assistant."
    )
    config["include_builtin_servers"] = True

    if tunnel:
        tunnel_cfg: dict[str, Any] = {"type": tunnel.get("type", "cloudflare")}
        if tunnel.get("command"):
            tunnel_cfg["command"] = tunnel["command"]
        config["tunnel"] = tunnel_cfg

    if channels:
        config["channels"] = channels

    config["serve"] = {"rate_limit_rpm": 60}

    # Try PyYAML for proper serialization; fall back to string construction.
    header = (
        f"# ollamacode.yaml — generated by setup wizard\n"
