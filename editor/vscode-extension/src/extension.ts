...
function log(msg: string): void {
  if (!_logChannel) _logChannel = vscode.window.createOutputChannel("OllamaCode");
...
function fetchOllamaModels(): Promise<string[]> {
  ...
  return new Promise((resolve) => {
    ...
    req.on("error", (err) => {
      log(`Ollama request error: ${(err as Error).message}`);
      finish([]);
    });
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("ollamacode.selectModel", async () => {
      ...
      const config = vscode.workspace.getConfiguration("ollamacode");
      const current = config.get<string>("model", "gpt-oss:20b");
...
class OllamaCodeChatProvider implements vscode.WebviewViewProvider {
  ...
  private _refreshChatView?: () => Promise<void>;
  ...
  resolveWebviewView(
    ...
    const setHtml = (models?: string[], current?: string, messages?: { role: string; content: string }[]) => {
    ...
    const setHtmlWithModels = async () => {
      ...
      const config = vscode.workspace.getConfiguration("ollamacode");
      const current = config.get<string>("model", "gpt-oss:20b");
      const messages = this._context.workspaceState.get<{ role: string; content: string }[]>("ollamacode.chatHistory") ?? [];
  ...
  async runQuery(query: string) {
    ...
    if (result.code === 143) {
      ...
      if (!result.text?.trim()) this.postMessage({ type: "append", role: "assistant", text: "(stopped)" });
  ...
  private async pushModelsToWebview() {
    let models: string[] = [];
    try {
      models = await fetchOllamaModels();
    } catch (e) {
      log(`pushModelsToWebview fetch error: ${(e as Error).message}`);
      models = [];
    }
    const config = vscode.workspace.getConfiguration("ollamacode");
    const current = config.get<string>("model", "gpt-oss:20b");
...
function getChatHtml(
  ...
  let selectOptionsHtml: string;
  if (hasInitialModels) {
    const opts = initialModels!.map(
      (name) => `<option value="${escapeHtml(name)}"${name === current ? " selected" : ""}>${escapeHtml(name)}</option>`
    );
...
function getComposerHtml(webview: vscode.Webview, nonce: string): string {
  const csp = `default-src 'none'; style-src 'nonce-${nonce}' 'unsafe-inline' ${webview.cspSource}; script-src 'nonce-${nonce}' ${webview.cspSource};`;
  return `<!DOCTYPE html>
<html>
<head>
